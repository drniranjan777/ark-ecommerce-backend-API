...
const createOrder = async (req) => {
  ...
  if (orderSource) {
    if (!req.session.buyNow.productId || !req.session.buyNow.quantity)
    ...
    const orderPrice = await calculateTotal(
      buyNowProduct.price * productSession.quantity
    );
...
const verifyPayment = async (req) => {
  ...
  const checkPaymentOrder = await Order.findOne({
...
const getUserOrders = async (req) => {
  ...
  const orderedProducts = await Promise.all(
    userOrders.map(async (order) => {
      const items = await OrderItem.find({ orderId: order.orderId }).populate(
        "productId"
      );
...
const orderAnalytics = async (req, res) => {
  ...
  const formattedSalesData = monthlyAnalytics.map((item) => ({
